...
\hat{y}_1 =\Big(1 + \exp \Big( -\beta_{01} - \sum_{m=1}^{64} \beta_{m1} max \Big( \gamma_{0m}\sum_{l=1}^{64} \gamma_{lm} max \Big( \alpha_{0l}\sum_{j=1}^{17}\alpha_{jl}x_j,0 \Big),0\Big) \Big)^{-1}\Big)
$$
Using Sigmoid activation on the output layer.
## c)

```{r}
# set.seed(1)
# train.in = sample(1:nrow(college.train), 0.8 * nrow(college.train))
# college.train1 = college.train[train.in, ]
# college.vali = college.train[-train.in, ]
# str(college.train1)
```
```{r}
model = keras_model_sequential() %>% 
  layer_dense(units = 64, activation = 'relu', input_shape = c(388)) %>% 
...
```{r}
set.seed(123)
history = model %>% fit(college.train$x,college.train$y,
epochs = 300,
batch_size = 8,
